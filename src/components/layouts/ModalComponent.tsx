...
const ModalComponent = ({isOpen, onOpenChange, children, onClose, closeButtonStroke = "white", modalContentClassName, backdropColor = "black"}:ModalType) => {
  ...
  return (
    <>
        <Modal shouldBlockScroll={false} isOpen={isOpen} onClose={onClose} onOpenChange={onOpenChange} scrollBehavior='inside' classNames={{backdrop: `bg-${backdropColor}`, wrapper: "items-end h-auto", base: "mt-auto" ,closeButton: ' z-3 hover:bg-transparent active:bg-transparent absolute appearance-none cursor-pointer select-none top-1 end-1 p-2 text-foreground-500 rounded-full tap-highlight-transparent outline-hidden data-[focus-visible=true]:z-10 data-[focus-visible=true]:outline-2 data-[focus-visible=true]:outline-focus data-[focus-visible=true]:outline-offset-2'}} 
        ...
        <path d="M23.9999 8.00006L8 24M7.99993 8L23.9999 23.9999" stroke={closeButtonStroke} strokeWidth="1.5" strokeLinecap="round"></path>
            ...
            <ModalContent className={`2xl:w-[78vw] 2xl:h-[auto] ${modalContentClassName} w-full  max-w-none max-h-full lg:rounded-md rounded-xl sm:my-0 sm:mx-0`}>
                {
                    () => (
                        <>
