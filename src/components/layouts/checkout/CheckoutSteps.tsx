...
const CheckoutSteps = () => {
  // Convertir el objeto en un array para poder mapearlo
  ...
  return (
    <div className='flex flex-col gap-[18px] md:gap-0 md:flex-row justify-between  md:items-center py-6 border-gradient-fixed-main mx-[var(--spacing-sm)]
        ...
        {stepsArray.map((stepName, index) => (
            <div key={index} className='flex text-center pb-[18px] mb-2 md:mb-0 md:pb-0 py-2 md:py-0 md:flex-col flex-row items-center md:!border-hidden border-gradient-fixed'>
                <span className='rounded-full w-8 h-8 flex justify-center items-center bg-gray-50'>{index + 1}</span>
                <h4 className='text-[18px] pl-3 md:pl-0 font-bold md:font-normal md:text-[clamp(12px,1.5vw,20px)]'>{stepName as string}</h4>
