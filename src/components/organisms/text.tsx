...
interface StepTabEntryFields extends EntrySkeletonType {
  internalName: string;
  entryBody: string;
  image?: Entry<{ internalName: string; image: Asset }>; // Updated type for nested image
  dot?: Asset;
}
    ...
    <div className=" items-center justify-items-center box-content lg:w-280 md:w-200 xsm:w-[320px] lg:h-80 md:h-80 xsm:h-[550px]">
      ...
      <div className=" relative  md:flex md:flex-row sm:flex-col ">
        ...
        {getCardsContent && getCardsContent.map((card: Entry<StepTabEntrySkeleton>) => {
          const { entryBody, image, dot } = card.fields;
          const imgURL = (image?.fields?.image as Asset)?.fields?.file?.url; // Access nested image URL
          const dotURL = (dot as Asset)?.fields?.file?.url; // Access dot URL
