...
import { BenefitsContainerProps } from "@/types/BenefitsTypes";
...
export default async function BenefitsComponent({ id }: BenefitsContainerProps) {
    ...
    console.log('benefit', benefitsEntry)

    const entryTitle = benefitsEntry?.fields.title as string;
    const entryCta = benefitsEntry?.fields.ctaText as string;
    // const entryData = benefitsEntry?.fields.tabs as unknown as EntrySkeletonType<TabsDataFields>;
    ...
    return (
        <div className="flex flex-col w-full self-center items-center bg-gray-50 text-black-0 gap-[40px] py-[64px]">
            ...
            <p>Cards</p>
            <ButtonGhost text={entryCta} classStyles="bg-black-0 py-[14px] px-[16px] rounded-md w-[320px] h-auto text-white-0 border-none font-semibold leading-[24px] text-lg" />
        </div>
    )
